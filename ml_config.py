...
ML_FEATURES = [
    ...
    "jet2_p",              # 动量第三大的喷注
    "jet3_p",              # 动量最小的那个喷注
    "Zcand_dm",            # 重建出的Z boson的mass与其实际物理质量91.19之间的绝对差值，重建是由两个qq重建的，因为在壳，正常来说这个值会趋于0， 【utils.h】
    ...
    "totalJetMass",        # 4个jets合成的总质量，对于信号来说，这个4 jets 理论上应该接近（Z质量 + W质量）                                【utils.h】
    "thrust",              # 所有粒子动量分布的各向异性（各向异性的程度），所有粒子动量分布的各向异性（各向异性的程度）                         【utils.h】
    "angleLepMiss",        # isolated lepton 与中微子之间的夹角。                                                                  【utils.h】
