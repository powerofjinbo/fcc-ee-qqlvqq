...
# [ML] Normalize class weights to equalize effective class influence while preserving intra-class shape info. This reduces class-imbalance bias and stabilizes gradient updates.
...
# [Model validation] Overtraining diagnostic between train/test score shapes; a large mismatch suggests model memorization or unstable feature behavior.
...
# [ML] Train with a holdout-validation split and early stopping; this adaptively chooses effective tree depth/number to avoid overfitting and unnecessary complexity.
...
# [ML] Hyperparameter grid search on a manageable subsample; early stopping keeps the search efficient while probing bias-variance trade-off directions.
...
# [ML + stats] OOF/K-fold scoring to obtain unbiased scores for all events, preventing train/test leakage into the final statistical fit.
